...
class RC5:
    ...
    def encryptFile(self, inpFileName, outFileName):
        with open(inpFileName, 'r') as inp, open(outFileName, 'wb') as out:
            run = True
            while run:
...
# Шифрование текста
def cipher_text(key,w,r,message):
    byte_array  = []
    byte_array = message.encode('utf-8')
    testRC5 = RC5(w, r, key)
    Cipher  =   testRC5.encryptBytes(byte_array)
    return Cipher
# Функции шифрования/дешифрования изображения
def img_byt_encimg(key,w,r):
    with Image.open('24пфон.png') as img:
        img = img.convert('RGB')
    byte_array  = []
...
def img_byt_decimg(key,w,r):
    with Image.open('encrypt.png') as img:
        img = img.convert('RGB')
    byte_array  = []
    byte_array = img.tobytes()
    testRC5 = RC5(w, r, key)
    Cipher  =   testRC5.decryptBytes(byte_array)
    img = Image.frombytes('RGB', (img.size), Cipher)
    img.save("decrypt.png")
key = b'\x78\x33\x48\xE7\x5A\xEB\x0F\x2F\xD7\xEB\x0F\x2F\xD7\xB1\x69'

print(cipher_text(key,64,12,'''Идет матч Франция—Россия. Комментатор: 
— Некому, некому дать пас!!! Зидан закрыт, Анри закрыт... Опытный игрок российской сборной Онопко закрыл их всех в раздевалке.
''').hex())
